...
// Data Access Object for feedback table — SQL only.
...
const FeedbackDAO = {
    ...
    findAll: async () => {
        const { rows } = await pool.query(`
            SELECT f.id, f.rating, f.comment AS comment_enc, f.visit_type, f.created_at,
                   u.fullname AS fullname_enc, e.name AS establishment_name_enc
    ...
    findByEstablishment: async (establishment_id) => {
        const { rows } = await pool.query(`
            ...
            ORDER BY f.created_at DESC`, [establishment_id]);
        return rows.map(_decryptRow);
    },
    ...
    findByRating: async (rating) => {
        const { rows } = await pool.query(`
            ...
            LEFT JOIN public.establishments e ON e.id = f.establishment_id
            WHERE f.rating = $1
            ORDER BY f.created_at DESC`, [rating]);
        return rows.map(_decryptRow);
    },

    // ── countAll (returns a number) ──────────────────────────────
    countAll: async () => {
        const { rows } = await pool.query(`SELECT COUNT(*) FROM public.feedback`);
        return parseInt(rows[0].count) || 0;
    },

    // ── countTotalFeedback — alias used by dashboardServiceImpl ──
    countTotalFeedback: async () => {
        return FeedbackDAO.countAll();
    },

    // ── getStats — full stats row for feedbackController ─────────
    getStats: async () => {
        const { rows } = await pool.query(`
            SELECT
    ...
    // ── getDistribution — per-star counts ────────────────────────
    ...
    // ── getFeedbackDistribution — alias used by dashboardServiceImpl
    getFeedbackDistribution: async () => {
        return FeedbackDAO.getDistribution();
    },

    // ── getCountByEstablishment — bar chart ───────────────────────
    getCountByEstablishment: async () => {
        const { rows } = await pool.query(`
            SELECT e.id, e.name AS establishment_name_enc, COUNT(f.id) AS feedback_count
    ...
    deleteById: async (id) => {
        const { rows } = await pool.query(
            `DELETE FROM public.feedback WHERE id = $1 RETURNING id`, [id]);
        return rows[0];
    },
};
